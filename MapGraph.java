...
public class MapGraph {
	...
	public void addEdge(GeographicPoint from, GeographicPoint to, String roadName,
		...
		//Create and initialise the MapEdge
		MapEdges ME = new MapEdges(from);
		ME.setDistance(length);
		ME.setEndPoint(to);
		ME.setRoadType(roadName);
		ME.setStreetName(roadName);
		//Find the MapNode (vertices.get(from)) at point from and add the street to
		//the arrayList of streets
		vertices.get(from).addStreet(ME);
		numberOfEdges += 1;
	}
	 ...
	 * @param start The starting location
	 * @param goal The goal location
	 * @param nodeSearched A hook for visualisation.  See assignment instructions for how to use it.
	 * @return The list of intersections that form the shortest (unweighted)
	 *   path from start to goal (including both start and goal).
	 */
			 					     ...
			 					     GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
		...
		while (nodesNeededToVisit.size() != 0) {
			...
			neighboursOfCurr = currNode.getNeighbours();
		...
		MapNode currNodeForList = vertices.get(goal);
		while(currNodeForList != vertices.get(start)) {
			shortestPathAsMapNode.add(currNodeForList);
			currNodeForList = path.get(currNodeForList);
		}
		shortestPathAsMapNode.add(vertices.get(start));
		Collections.reverse(shortestPathAsMapNode);
				
		// Hook for visualization.  See writeup.
		//nodeSearched.accept(next.getLocation());
		
		return convertListOfMapNodesToListOfGeographicPoints(shortestPathAsMapNode);
	}
	
	//Helper method to convert list of MapNodes into list of GeographicPoints
	public List<GeographicPoint> convertListOfMapNodesToListOfGeographicPoints(List<MapNode> list){
		ArrayList<GeographicPoint> ListAsGeographicPoint = new ArrayList<GeographicPoint>();
		for(int i = 0; i < list.size(); i++) {
			ListAsGeographicPoint.add((list.get(i).getNodeAsGP()));
		}
		return ListAsGeographicPoint;
	}
